...
void score_it(char *results_file_name)  
{
	...
	A=mlfw_mat_double_create_column_vec(dataset,dataset_columns-2);  // 2nd last column of the file
	if(A==NULL)
	{
		printf("Low memory\n");
	...
	P=mlfw_mat_double_create_column_vec(dataset,dataset_columns-1); // last column of the file
	if(P==NULL)
	{
		printf("Low memory\n");
	        mlfw_mat_double_destroy(dataset);
	...
	R=mlfw_substract_double_column_vector(A,P);
	if(R==NULL)
	{
		printf("Low memory\n");
	        mlfw_mat_double_destroy(dataset);
	...
	AM=mlfw_substract_double_column_vector(A,M);
	if(AM==NULL)
	{
		printf("Low memory\n");
	        mlfw_mat_double_destroy(dataset);
